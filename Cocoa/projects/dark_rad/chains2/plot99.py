...
parameter = [u'omegam', u'theta_MC_100',u'H0', u'w', u'wa',u'chi2__bao.desi_y1',u'chi2__sn.pantheon']
chaindir=r'./'
...
samples.saveAsText(chaindir + '/.VM_P99_TMP1')
# --------------------------------------------------------------------------------
samples=loadMCSamples(chaindir + root_chains[1],settings=analysissettings)
p = samples.getParams()
samples.addDerived(p.s8omegamp5/0.5477225575,name='SS8',label='{S_8}')
samples.saveAsText(chaindir + '/.VM_P99_TMP2')

#GET DIST PLOT SETUP
g=gplot.getSubplotPlotter(chain_dir=chaindir,
analysis_settings=analysissettings2,width_inch=11.0)
...
param_3d = None
g.triangle_plot(['.VM_P99_TMP1','.VM_P99_TMP2'],parameter,
plot_3d_with_param=param_3d,line_args=[
{'lw': 1.1,'ls': 'solid', 'color':'royalblue'},
{'lw': 1.9,'ls': 'dashed', 'color':'firebrick'},
{'lw': 1.9,'ls': 'solid', 'color':'black'}],
contour_colors=['royalblue','firebrick','black'],
filled=[True,True,False],shaded=False,
legend_labels=[
  'Planck $\\ell < 1200$ + DESI + Pantheon', 
  'Planck $\\ell < 2500$ + DESI + Pantheon',
#  'Planck $\\ell < 2500$ + DESI + Pantheon NPH'
],
legend_loc=(0.45,0.85),
imax_shaded=0)
g.export()