...
parameter = [u'omegam2', u'H0', u'SS8',u'n_s',u'As_1e9',
u'omegabh2',u'sigma8',u'Omega0_scf_ke',u'chi2__SN',u'chi2__CMB']
chaindir=r'./'

analysissettings={'smooth_scale_1D':0.45,'smooth_scale_2D':0.45,
'ignore_rows': u'0.75','range_confidence' : u'0.025'}
...
samples.saveAsText(chaindir + '/.VM_PKIMv2_TMP1')
# --------------------------------------------------------------------------------
samples=loadMCSamples(chaindir + root_chains[1],settings=analysissettings)
samples.thin(factor = int(np.sum(samples.weights)/num_points_thin))
p = samples.getParams()
samples.addDerived(p.omegam,name='omegam2',label='{\\Omega_m}')
samples.addDerived(p.s8omegamp5/0.5477225575,name='SS8',label='{S_8}')
samples.saveAsText(chaindir + '/.VM_PKIMv2_TMP2')
...
param_3d = None
g.triangle_plot(['.VM_PKIMv2_TMP1','.VM_PKIMv2_TMP2'],parameter,
