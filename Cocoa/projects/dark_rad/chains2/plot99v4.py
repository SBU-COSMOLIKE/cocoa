...
parameter = [u'omegam', u'H0', u'w', u'wa', u'SS8',u'chi2__bao.desi_y1',u'mnu']
chaindir=r'./'
...
samples.saveAsText(chaindir + '/.VM_P99v4_TMP1')
# --------------------------------------------------------------------------------
samples=loadMCSamples(chaindir + root_chains[1],settings=analysissettings)
samples.thin(factor = int(np.sum(samples.weights)/num_points_thin))
p = samples.getParams()
samples.addDerived(p.s8omegamp5/0.5477225575,name='SS8',label='{S_8}')
samples.saveAsText(chaindir + '/.VM_P99v4_TMP2')
# --------------------------------------------------------------------------------
...
param_3d = None
g.triangle_plot(['.VM_P99v4_TMP1',
                 '.VM_P99v4_TMP2'],parameter,
plot_3d_with_param=param_3d,
legend_labels=[
  'Planck $\\ell < 2500$ + DESI + Pantheon ($m_{nu}$)', 
  'Planck $\\ell < 2500$ + DESI + Pantheon ($m_{nu}$) w>-1,w0pwa-1', 
],
legend_loc=(0.3,0.835),
imax_shaded=0,
