...
num_points_thin = 100000

parameter = [u'omegam2', u'H0', u'SS8',u'm_ncdm',u'n_s',u'As_1e9',u'sigma8',u'scf_c']
chaindir=r'./'
...
samples.saveAsText(chaindir + '/.VM_PKIM3_TMP1')
# --------------------------------------------------------------------------------
...
samples.addDerived(p.sigma8*np.sqrt(p.omegan+(p.omegach2+p.omegabh2)/(p.H0/100.)/(p.H0/100.))/0.5477225575,name='SS8',label='{S_8}')
samples.saveAsText(chaindir + '/.VM_PKIM3_TMP2')
# --------------------------------------------------------------------------------
...
samples.addDerived(p.omegan+(p.omegach2+p.omegabh2)/(p.H0/100.)/(p.H0/100.),name='omegam2',label='{\\Omega_m}')#sigma8
samples.addDerived(p.sigma8*np.sqrt(p.omegan+(p.omegach2+p.omegabh2)/(p.H0/100.)/(p.H0/100.))/0.5477225575,name='SS8',label='{S_8}')
samples.saveAsText(chaindir + '/.VM_PKIM3_TMP3')
...
param_3d = None
g.triangle_plot(['.VM_PKIM3_TMP1','.VM_PKIM3_TMP2','.VM_PKIM3_TMP3'],parameter,
...
legend_labels=[
  'Planck $\\ell < 1296$ + Pantheon, LINEAR SCF (MNU)', 
  'Planck $\\ell < 1296$ + DESI + Pantheon, LINEAR SCF (MNU)', 
  'Planck $\\ell < 1296$ + DESI + Pantheon (no z=0.5, 0.7) + BOSS (no z=0.3), LINEAR SCF (MNU)', 
],
